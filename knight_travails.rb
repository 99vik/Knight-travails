...
class Game
  ...
  def initialize
    ...
    loop do
      self.start_position = PositionNode.new((board.select { |_key, value| value == 'N' }.flatten[0]))
      move_knight
    end
  end
  ...
  def change_board_position(path)
    path_display = [start_position.position]
    path.each do |i|
      path_display << i
      board[board.find { |_key, value| value == 'N' }[0]] = nil
      board[i] = 'N'
      draw_board
      puts
  ...
  def find_path(target)
    return [] if target == start_position.position

    queue = []
    possible_moves(start_position.position).each { |i| queue.push(PositionNode.new(i, start_position)) }
  ...
  def possible_moves(position)
    ...
    moves.reject { |i| PositionNode.history.include?(i) }
  end
  ...
  def select_move_position(input)
    ...
    return wrong_input unless input_valid?(input)
  ...
  def wrong_input
    ...
    select_move_position(gets.strip)
  end
